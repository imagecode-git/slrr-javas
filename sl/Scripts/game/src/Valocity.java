...
{
	//RAXAT: multipliers for making up traffic jams in the city
	public static int	TRAFFIC_DENSITY_MUL_RUSH_HOUR		= 5;
	public static int	TRAFFIC_DENSITY_MUL_BUSINESS_LUNCH	= 3;
	public static int	TRAFFIC_DENSITY_MUL_WEEKEND			= 2;
	public static int	TRAFFIC_DENSITY_MUL_NORMAL			= 1;
	
	
    //keep track of it so it can be renewed when entering a higher club
    public Valocity(Multiplayer mp)
    {
    ...
    public void enter( GameState prev_state )
    {
        ...
        {
			Frontend.loadingScreen.show(new ResourceRef(frontend:0x000002E7r));
			GfxEngine.flush();
...
//				posStart = map.getNearestCross( posStart );
			...
			int trafficMultiplier = 1; //RAXAT: traffic jams and more pedestrians in peak hours, less traffic in daytime
			
			if((GameLogic.day-1)%7 < 5) //business days
			{
				if(hour > 8 && hour < 11 || hour > 17 && hour < 20) trafficMultiplier = TRAFFIC_DENSITY_MUL_RUSH_HOUR; //rush hour traffic jam
				if(hour > 12 && hour < 14) trafficMultiplier = TRAFFIC_DENSITY_MUL_BUSINESS_LUNCH; //business lunch
			}
			else //weekends
			{
				if(hour < 11) trafficMultiplier = TRAFFIC_DENSITY_MUL_NORMAL;
				else trafficMultiplier = TRAFFIC_DENSITY_MUL_WEEKEND;
			}
			
			if( hour > 4 && hour < 22 )
			{
				//day time settings
				map.addTraffic(new GameRef(cars.traffic.Taxi:0x00000006r), 40*trafficMultiplier, 2, 5, 2);
				map.addTraffic(new GameRef(cars.traffic.Ambulance:0x00000006r), 10*trafficMultiplier, 2, 5, 2);
				map.addTraffic(new GameRef(cars.traffic.FireEngine:0x00000006r), 12, 2, 5, 2);
				map.addTraffic(new GameRef(cars.traffic.Coach:0x00000006r), 15*trafficMultiplier, 2, 10,2);

				map.addTraffic(new GameRef(cars.traffic.Schoolbus:0x00000006r), 30*trafficMultiplier, 2, 10, 2);
				map.addTraffic(new GameRef(cars.traffic.ArmoredVan:0x00000006r), 20, 2, 5, 2);
				map.addTraffic(new GameRef(cars.traffic.Wagon:0x00000006r), 100*(trafficMultiplier/2), 2, 5, 2);
				
				map.addTraffic(new GameRef(cars.traffic.Erbilac:0x00000006r), 50*(trafficMultiplier/2), 2, 5, 2);
				map.addTraffic(new GameRef(cars.traffic.Erbilac_2:0x00000006r), 50*(trafficMultiplier/2), 2, 5, 2);
				map.addTraffic(new GameRef(cars.traffic.Erbilac_3:0x00000006r), 50*(trafficMultiplier/2), 2, 5, 2);
				
				map.addTraffic(new GameRef(cars.traffic.CivilVan:0x00000006r), 50*(trafficMultiplier/2), 2, 5, 2);
				map.addTraffic(new GameRef(cars.traffic.CivilVan_2:0x00000006r), 50*(trafficMultiplier/2), 2, 5, 2);
				map.addTraffic(new GameRef(cars.traffic.CivilVan_3:0x00000006r), 50*(trafficMultiplier/2), 2, 5, 2);
				
				map.addTraffic(new GameRef(cars.traffic.Peterbild:0x00000006r), 25*trafficMultiplier, 2, 5, 2);
				map.addTraffic(new GameRef(cars.traffic.Peterbild_2:0x00000006r), 25*trafficMultiplier, 2, 5, 2);
				map.addTraffic(new GameRef(cars.traffic.Peterbild_3:0x00000006r), 25*trafficMultiplier, 2, 5, 2);
				
				map.addTraffic(new GameRef(cars.traffic.Pickup:0x00000006r), 40*trafficMultiplier, 2, 7, 2);
				map.addTraffic(new GameRef(cars.traffic.Pickup_2:0x00000006r), 40*trafficMultiplier, 2, 7, 2);
				map.addTraffic(new GameRef(cars.traffic.Pickup_3:0x00000006r), 40*trafficMultiplier, 2, 7, 2);
					...
					for( int i=0; i<GameLogic.speedymen.length; i++ )
					{
						if( Math.random() > 0.2 )
						{
							...
							if( opp instanceof Bot )
							{
								...
								{
									//preloaded bots
									botopp.dummycar = (GameRef)botopp.getCar(map); //RAXAT: v2.3.1, loaded vehicles instead of generated ones
									
									//RAXAT: build 937, dummycar loading patch
									int idx = (int)((nrData.startPos.length-1)*Math.random());
									Vector3 pos = map.getNearestCross(nrData.startPos[idx]).mul(Math.random()*20.0);
									
									botopp.traffic_id = map.addTrafficCar(botopp.dummycar, pos);
									map.notifyTrafficCar(botopp.traffic_id, 1);
									
									//classic bots (randomly generated vehicles)
									/*
				...
				if( GameLogic.gameMode == GameLogic.GM_DEMO ) map.addTraffic( GRT_POLICECAR, 50, 2, 5, 2);

				map.setPedestrianDensity( 0.003*trafficMultiplier );
			}
			else
			{
				//night time settings
